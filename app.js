const DATA={substations:[],equipment:{},histories:{},maintenanceTypes:['Manutenção corretiva','Manutenção preventiva','Apoio em serviço de subestação'],meta:{source:'Supabase',version:'1.9.0'}};
...
function assertLocalRuntimeDependencies(){
  ...
  if(missing.length)throw new Error(`Dependências locais ausentes: ${missing.join(', ')}. Execute PREPARAR_RELEASE.bat antes de publicar/instalar a v1.9.0.`);
}
...
enterApplication=async function(...args){await _v120EnterApplication(...args);const version=document.getElementById('app-version-label');if(version)version.textContent='v1.9.0';setTimeout(()=>reconcilePushRegistrationSilently(),300)};
...
enterApplication=async function(...args){
  ...
  const footerVersion=document.getElementById('environment-footer-version');if(footerVersion)footerVersion.textContent='v1.9.0';
};
...
enterApplication=async function(...args){await _v150EnterApplication(...args);const footerVersion=document.getElementById('environment-footer-version');if(footerVersion)footerVersion.textContent='v1.9.0';const version=document.getElementById('app-version-label');if(version)version.textContent='v1.9.0'};
...
enterApplication=async function(...args){
  ...
  const version=document.getElementById('app-version-label');if(version)version.textContent='v1.9.0';
  const footerVersion=document.getElementById('environment-footer-version');if(footerVersion)footerVersion.textContent='v1.9.0';
