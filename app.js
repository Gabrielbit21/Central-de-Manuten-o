const DATA={substations:[],equipment:{},histories:{},maintenanceTypes:['Manutenção corretiva','Manutenção preventiva','Apoio em serviço de subestação'],meta:{source:'Supabase',version:'2.0.5'}};
...
function assertLocalRuntimeDependencies(){
  ...
  if(missing.length)throw new Error(`Dependências locais ausentes: ${missing.join(', ')}. Execute PREPARAR_RELEASE.bat antes de publicar/instalar a v2.0.5.`);
}
...
enterApplication=async function(...args){await _v120EnterApplication(...args);const version=document.getElementById('app-version-label');if(version)version.textContent='v2.0.5';setTimeout(()=>reconcilePushRegistrationSilently(),300)};
const APP_BUILD='2.0.5';
...
enterApplication=async function(...args){
  ...
  const footerVersion=document.getElementById('environment-footer-version');if(footerVersion)footerVersion.textContent='v2.0.5';
};
...
enterApplication=async function(...args){await _v150EnterApplication(...args);const footerVersion=document.getElementById('environment-footer-version');if(footerVersion)footerVersion.textContent='v2.0.5';const version=document.getElementById('app-version-label');if(version)version.textContent='v2.0.5'};
...
enterApplication=async function(...args){
  ...
  const version=document.getElementById('app-version-label');if(version)version.textContent='v2.0.5';
  const footerVersion=document.getElementById('environment-footer-version');if(footerVersion)footerVersion.textContent='v2.0.5';
...
/* ===== v2.0.5 — fundação multi-frente e equipe executante padronizada ===== */
...
async function loadV200Auxiliary(){
  ...
  try{
    ...
    console.warn('v2.0.5 diretório de colaboradores:',error?.message||error);
    const cached=await idbGet('cloudCache',key);
...
ensureReportChildren=async function(record,user){
  ...
  try{
    ...
    console.warn('v2.0.5 participantes:',error?.message||error);
...
/* ===== v2.0.5 — refinamentos visuais, histórico seguro e vínculo de técnicos ===== */
...
async function v201FullAssetHistory(asset,subId=state.sub){
  ...
  if(navigator.onLine&&cloudClient&&state.cloudUser){
    try{
      ...
      console.warn('v2.0.5 histórico do ativo:',error?.message||error);
    }
  }
