...
/* ===== v3.1.0 — atualização em massa multi-frente no mesmo componente ===== */
...
/* ===== fim v3.1.0 atualização em massa multi-frente ===== */
...
function centralValidationContainer(target){
  ...
  if(target.classList?.contains('field')||target.classList?.contains('auth-field')||target.classList?.contains('v205-check-field')||target.classList?.contains('v200-team-field'))return target;
  return target.closest?.('.field,.auth-field,.v205-check-field,.v200-team-field')||target;
}
function centralValidationApplicable(target){
  if(!target||target.disabled||target.type==='hidden')return false;
  if(target.closest?.('[hidden],.hidden'))return false;
  return true;
}
function centralClearValidationTarget(target){
  const box=centralValidationContainer(target);
  if(!box)return;
  box.classList.remove('v312-invalid');
  box.querySelectorAll('.v312-validation-message').forEach(node=>node.remove());
  box.querySelectorAll('[aria-invalid="true"]').forEach(node=>node.removeAttribute('aria-invalid'));
}
function centralMarkValidationTarget(target,message='Este campo é obrigatório.'){
  const box=centralValidationContainer(target);
  if(!box)return box;
  box.classList.add('v312-invalid');
  const control=target.matches?.('input,select,textarea')?target:box.querySelector('input:not([type="hidden"]),select,textarea');
  control?.setAttribute('aria-invalid','true');
  let note=box.querySelector(':scope > .v312-validation-message');
  if(!note){note=document.createElement('small');note.className='v312-validation-message';box.appendChild(note)}
  note.textContent=message||'Revise este campo.';
  return box;
}
function centralScrollToValidationTarget(target){
  const box=centralValidationContainer(target)||target;if(!box)return;
  try{box.scrollIntoView({behavior:'smooth',block:'center',inline:'nearest'})}catch(_){box.scrollIntoView()}
  const focusable=target?.matches?.('input,select,textarea,button')?target:box.querySelector('input:not([type="hidden"]):not(:disabled),select:not(:disabled),textarea:not(:disabled),button:not(:disabled)');
...
function centralValidateForm(form,{checkGroups=true}={}){
  if(!form)return true;centralInstallValidationCleanup(form);form.querySelectorAll('.v312-invalid').forEach(node=>centralClearValidationTarget(node));const invalid=[];
...
/* ===== v3.1.0 — exportação multi-frente no MESMO componente ===== */
...
/* ===== fim v3.1.0 exportação multi-frente ===== */
...
/* ===== v1.9.9 — atmosfera visual da Home ===== */
const _v199SetActiveNav=setActiveNav;
...
/* ===== v3.1.0 — fundação multi-frente e equipe executante padronizada ===== */
...
async function loadV200Auxiliary(){
  ...
  try{
    ...
    console.warn('v3.1.0 diretório de colaboradores:',error?.message||error);
    const cached=await idbGet('cloudCache',key);
...
submitMaintenance=async function(ev,form,sel){
  if(!(state.maintenanceParticipants||[]).length){
    ev?.preventDefault?.();
    const field=form?.querySelector('.v200-team-field')||document.getElementById('v200-team-select');
    centralMarkValidationTarget(field,'Selecione pelo menos um integrante da equipe executante.');toast('Selecione pelo menos um integrante da equipe executante.','warning');centralScrollToValidationTarget(field);return;
  }
  centralClearValidationTarget(form?.querySelector('.v200-team-field'));
...
ensureReportChildren=async function(record,user){
  ...
  try{
    ...
    console.warn('v3.1.0 participantes:',error?.message||error);
...
/* ===== v3.1.0 — refinamentos visuais, histórico seguro e vínculo de técnicos ===== */
...
async function v201FullAssetHistory(asset,subId=state.sub){
  ...
  if(navigator.onLine&&cloudClient&&state.cloudUser){
    try{
      ...
      console.warn('v3.1.0 histórico do ativo:',error?.message||error);
    }
  }
...
globalThis.__CENTRAL_BOOT__=async()=>{  await registerCentralServiceWorker();
  if(await ensureCurrentBuild())bootConnectedApp();
};
if(!globalThis.__CENTRAL_DEFER_BOOT__)globalThis.__CENTRAL_BOOT__();
