...
(() => {
  ...
  function checkRequiredGroups(form){let ok=true;form.querySelectorAll('[data-check-required="true"]').forEach(g=>{const any=[...g.querySelectorAll('input[type=checkbox]')].some(x=>x.checked);g.classList.toggle('v205-invalid',!any);if(!any)ok=false});if(!ok)toast('Preencha os grupos obrigatórios.','warning');return ok}
  ...
  async function renderV205Activity(){
    ...
    form.onsubmit=async e=>{e.preventDefault();form.querySelector('.v205-cadastral')?._v205Sync?.();validateMaskedFields(form);if(!form.reportValidity()||!checkRequiredGroups(form))return;const values=formValues(form);await renderV205Review(asset,values)};
  }
