/* Central de Manutenção SE — v3.1.3
...
function v300SyncVersionChrome(){
  ...
  if(footer)footer.textContent='v3.1.3';
  const label=document.getElementById('app-version-label');
  if(label)label.textContent='v3.1.3';
  if(document.title&&/v\d+\.\d+\.\d+/i.test(document.title)){
    document.title=document.title.replace(/v\d+\.\d+\.\d+/ig,'v3.1.3');
  }
}
