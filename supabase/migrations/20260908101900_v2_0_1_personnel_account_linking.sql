...
-- Vínculo canônico entre conta da Equipe de Campo e cadastro de técnico.

create unique index if not exists profiles_personnel_id_unique
...
begin
  ...
  if coalesce(v_profile.requested_role, v_profile.role, 'field') <> 'field'
     and coalesce(v_profile.role, 'field') <> 'field' then
...
'Vincula uma conta de campo ao cadastro canônico de técnico. Exige administrador ativo e impede que o mesmo técnico seja ligado a duas contas.';
